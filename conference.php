...
Plugin URI: https://www.ablion.in
...
Author: ABLION
Author URI: https://www.ablion.in
Version: 2.2
*/
...
function wcp_load_admin_MetaBoxes() {                
    ...
    add_meta_box("wcp_conference_speakers_design", "Speakers Page Design", "wcp_speakers_design_MetaBox", "conference", "side", "low");
...
function wcp_speakers_design_MetaBox() {
    global $post;
    
    $designs = array(1 => "Design 1", 2 => "Design 2");        
    $des = get_post_meta($post->ID, 'speakers_design', true);    
                
    if(!empty($designs)) {
        foreach($designs as $key => $design) {            
            $checked = '';
            if(!$des) {
                // If no design is selected, choose design 1 as default
                if($key == 1) {
                    $checked = 'checked="checked"';    
                }
            } else {
            ...
            echo '<p><label><input type="radio" name="speakers_design" value="'.$key.'" '.$checked.'/>'.$design.'</label></p>';
        }
    }
}
...
function wcp_save_Conference(){
    ...
    $conference_data['speakers_design'] = isset( $_POST['speakers_design'] ) ? $_POST['speakers_design'] : "1";
